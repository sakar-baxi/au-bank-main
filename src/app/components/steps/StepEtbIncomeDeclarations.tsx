...
export default function StepEtbIncomeDeclarations() {
  ...
  const [incomeRange, setIncomeRange] = useState(formData.incomeRange || "10-15L");
  ...
  const [grossAnnualIncome, setGrossAnnualIncome] = useState(
    formData.grossAnnualIncome || formData.income || ""
  );
  ...
  const [isTaxResidentIndiaOnly, setIsTaxResidentIndiaOnly] = useState<boolean>(
    formData.isTaxResidentIndiaOnly !== false
  );
  ...
  return (
    <StepCard step={stepLabel} maxWidth="6xl">
      ...
      {isEtbNk && (
        <AgentBanner>
          We need a few income declarations and consents to meet regulatory requirements before we
      ...
      <div className="page-header mb-2">
        ...
        <p className="page-subtitle text-slate-600 mt-1">
          Select income range and confirm declarations.
        </p>
      </div>
        ...
        <div className="rounded-xl border border-slate-200/80 bg-slate-50/50 p-5 space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-600 mt-1">
              As per RBI/Compliance requirements, please confirm the below.
            </p>
          </div>
