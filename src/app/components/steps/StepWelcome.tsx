...
export default function StepWelcome() {
  ...
  // Welcome step field layout (matches HDFC source of truth):
  // NTB, NTB-conversion, ETB-NK, ETB: show mobileNumber, dob, pan
  //   - NTB: mobile+dob disabled (prefilled); pan editable (demo note shown)
  //   - ETB / ETB-NK: mobile+dob+pan all disabled (prefilled from invite)
  // All other journey types: show name + mobileNumber
  const showTriFields = journeyType === "ntb" || journeyType === "ntb-conversion" || journeyType === "etb-nk" || journeyType === "etb";

  // Returns true when a field should be read-only/disabled because it is prefilled from the invite.
  // Matches HDFC StepWelcome disabled rules exactly:
  //   mobile & dob  → disabled for all salary account journeys
  //   pan           → disabled for etb and etb-nk; editable for ntb/ntb-conversion (demo note)
  const isPrefilledField = (fieldId: string): boolean => {
    if (fieldId === "mobileNumber" || fieldId === "dob") {
      return journeyType === "ntb" || journeyType === "ntb-conversion" || journeyType === "etb-nk" || journeyType === "etb";
    }
    if (fieldId === "pan") {
      return journeyType === "etb" || journeyType === "etb-nk";
    }
    return false;
  };
  ...
  const welcomeFields = useMemo(() => {
    if (showTriFields) {
      return [
        { id: "mobileNumber", label: "Mobile Number", source: "bank-api" as const },
        { id: "dob", label: "Date of Birth", source: "bank-api" as const },
        { id: "pan", label: "PAN Number", source: "bank-api" as const },
      ] as FieldConfig[];
    }
  ...
  }, [journeyConfig.stepFieldLayouts?.welcome, showTriFields]);
    ...
    <StepCard step={stepLabel} maxWidth="6xl">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
          ...
          const isPrefilled = isPrefilledField(field.id);
