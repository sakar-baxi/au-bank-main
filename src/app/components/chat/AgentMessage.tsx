...
export default function AgentMessage({ children, isNew = true }: AgentMessageProps) {
    return (
        <div className={`flex items-start gap-2 mb-4 ${isNew ? 'animate-in slide-in-from-left-4 duration-300' : ''}`}>
            {/* Agent Avatar */}
            <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ backgroundColor: "rgba(var(--primary-bank-rgb, 66, 38, 94), 0.15)", color: "var(--primary-bank)" }}>
